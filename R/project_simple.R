...
populate_project_simple <- function( batch = FALSE ) {
  ...
  # returned_object_metadata <- redcap_metadata_export(redcap_uri=uri, token=token)
  # write.csv(returned_object_metadata$data, file="./inst/test_data/simple_metadata.csv", row.names=FALSE)
