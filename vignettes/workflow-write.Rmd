...
A data.frame is returned, with one row per potential problem (and a suggestion how to avoid it).
If you encounter importing problems that can be checked with automation, 
...
```{r patient-validate}
REDCapR::validate_for_write(ds_patient)
```
