...
This vignette covers the the basic functions exposed by the [`httr`](https://github.com/OuhscCcanMiechvEvaluation/httr) and [`curl`](http://cran.r-project.org/package=curl) packages which allow you to interact with [REDCap](http://www.project-redcap.org/) through its API.

## Reading REDCap Data
The function `redcap_read_oneshot` uses the [`httr`](http://cran.r-project.org/package=httr) package to call the REDCap API.
