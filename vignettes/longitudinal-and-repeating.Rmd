...
When provided a structure like Table 5, some people's initial attempt involves breaking up Table 5 so it resembles Tables 1, 3a, & 3b.  Although this can be done, the code is difficult to maintain and not portable to projects with different instruments.  (The code is really ugly too.)

Our advice is to walk upstream of Table 5 and modify how the information is retrieved.  Like other database systems, request the three tables separately from the server and then combine them on your desktop to fit your analyses.

These two approaches are good for most scenarios: (a) a single call to REDCapTidieR's [REDCapTidieR::redcap_read_tidy()] or (b) multiple calls to REDCapR's [redcap_read()].
...
We recommend calling [REDCapTidieR](https://chop-cgtdataops.github.io/REDCapTidieR/) a single time if
...
Calling [REDCapR](https://ouhscbbmc.github.io/REDCapR/) multiple times requires more development time than calling REDCapTidieR once, but could be worth your effort if:
...
If in doubt, start with REDCapTidieR.  Escalate to REDCapR if the operation is taking too long and can be decreased by reducing the information retrieved from the server and transported across the network.

Escalating to REDCapR
------------------------------------------------------------------

Even if you know that you'll want REDCapR's low-level control, consider starting with REDCapTidieR anyway.  ...particularly if you are unsure how to specify the grains of the table.  The structure of REDCapTidieR's tables are a great starting point for most conventional analyses.  If you need the performance of REDCapR but are unsure how the tables should look, simply execute something like `REDCapTidieR::redcap_read_tidy(url, project_token)` and study its output.  Then try to mimic it exactly with `REDCapR::redcap_read()` calls.
