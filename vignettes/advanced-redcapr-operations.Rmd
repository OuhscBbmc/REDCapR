...
When widening more than one variable (*e.g.*, `pmq1` - `pmq4`), it's usually easiest to go even longer/taller (*e.g.*, `ds_eav`) before reversing direction and going wide.  This example contains the additional wrinkle that the REDCap event names "first_dose" and "first_visit" are renamed "dose_1" and "visit_1", which wlil help all the values be dose and visit values be proper numbers.
