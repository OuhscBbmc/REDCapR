...
* `redcap_read()` and `redcap_read_oneshot()` accept a delimiter, to accommodate server instances in locales that use something other than a comma.  The delimiter is passed both to the API call and to `readr::read_delim()` (#570, @mrkskk)
* `redcap_event_instruments()`, `redcap_event_read()`, `redcap_instruments()`, `redcap_project_info_read()`, and `redcap_variable()` accept a delimiter, to accommodate server instances in locales that use something other than a comma.  The delimiter is passed  only to `readr::read_delim()` (#570, @mrkskk)
